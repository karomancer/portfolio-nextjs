...
Dan came in with some ideas of his own. One was the game's cosmology in three dimensions: a digital version of an [orrery](https://en.wikipedia.org/wiki/Orrery). The other was a gnomish contraption I likened to an atompunk device, with multiple panels for primary information, effect intensity, a lunar orbit diagram, and a forecast list.
...
* write parsers and collections for reading in game data in an arbitrarily defined format in `.dat` files
* write CLI commands for operations, such as finding the coordinates of a location from within the greater map and generating thumbnails of what that looks like on the map (for the worldgate tiles)
